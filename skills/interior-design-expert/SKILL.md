...
- AI-assisted room visualization with Nano Banana (Gemini), via the **interior-redesign-studio** skill
...
- 3D modeling implementation → use Archicad/Blender directly

## Visualization & Rendering

This skill covers the **design thinking** (space planning, color, lighting, proportion). For
photoreal renders, hand off to the dedicated rendering skill — do not call image MCPs directly.

| Tool | Purpose |
|------|---------|
| **interior-redesign-studio** skill | Photoreal renders & edits via Nano Banana (Google Gemini image). The actual rendering engine. |
| **archicad-fa-pro** skill / Tapir MCP | Pull real dimensions and the base 3D view from an Archicad/BIM model. |
| **Blender MCP** (if configured) | Direct 3D modeling/render control. |

### Room Visualization Workflow
```
1. Establish room parameters here (dimensions, style, palette, lighting plan).
   - If the source is an Archicad/BIM model, pull true dimensions first.
2. Hand the brief to the interior-redesign-studio skill to render (Phase 1 photoreal → Phase 2 humanize).
3. Iterate via that skill's presets (edits, lighting variants, style variations).
```
Note: derive the style from the analyzed space; do not impose a fixed preset unless the user names one.
...
- **Archicad / Blender** - 3D model & base views; **Nano Banana (Gemini)** via interior-redesign-studio - photoreal renders
